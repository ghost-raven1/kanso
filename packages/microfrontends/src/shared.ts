...
import * as overlays from '@kanso/core/overlays';
...
export function sharedModules(extra: Record<string, { version: string; module: object }> = {}) {
  const modules: Record<string, object> = { 'solid-js': solid, 'solid-js/web': web, 'solid-js/store': store, '@kanso/core': core, '@kanso/core/internal': internal, '@kanso/core/client': client, '@kanso/core/overlays': overlays, '@kanso/app': app, '@kanso/app/integration': integration, '@kanso/app/seo': seo, '@solidjs/router': router, '@kanso/app/solid-router': router };
  ...
  const version = (name: string) => Object.entries(RUNTIME_VERSIONS).find(([prefix]) => name === prefix || name.startsWith(prefix + '/'))?.[1] ?? '0.10.0';
