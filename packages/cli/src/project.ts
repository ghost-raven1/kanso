import { readFile, readdir, writeFile, rename, rm, stat, realpath } from 'node:fs/promises';
...
export async function dependsOnReact(root: string, name: string, seen = new Set<string>()): Promise<boolean> {
  if (name.startsWith('@kanso/')) return false;
...
export async function migrate(options: MigrationOptions): Promise<MigrationReport> {
  ...
  try { resolver = await createProjectResolver(root); }
  catch (caught) { const message = String(caught); error(root, message.match(/[A-Z_]+:/)?.[0].slice(0, -1) ?? 'CONFIG', message); return { diagnostics, changes, applied: false, modules: 0 }; }
  ...
  const visit = async (file: string): Promise<void> => {
    ...
    try {
      ...
      if (file === resolver.configFile && resolver.paths) result.code = enableTsconfigPaths(result.code, file);
  ...
  for (const file of await readdir(root)) {
    ...
    if (/^tsconfig.*\.json$/.test(file)) {
      ...
      if (parsed.config.compilerOptions?.jsx || file === 'tsconfig.json' && resolver.compilerOptions.jsx) {
  ...
  if (needsMigration) {
    ...
    const version = (name: string) => options.local ? `file:${resolve(options.local, 'packages', name)}` : '^0.4.0';
