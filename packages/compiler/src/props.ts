...
import { hasReactive, tracked, isSetup, importedName, type TransformContext } from './context.js';
...
export function transformLocalProps(context: TransformContext): void {
  context.program.traverse({ VariableDeclarator(path) {
    ...
    if (t.isCallExpression(init) && t.isIdentifier(init.callee) && ([...context.helpers.values()].some(value => value.name === (init.callee as t.Identifier).name)
