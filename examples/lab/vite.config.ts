...
const dataServer: Plugin = {
  ...
  configureServer(server) {
    server.middlewares.use(async (req, res, next) => {
      if (
        !req.url?.startsWith('/_kanso/') &&
        !['/robots.txt', '/sitemap.xml'].includes(req.url ?? '')
      )
        return next();
      try {
        const module = await server.ssrLoadModule('/server.ts');
        await nodeHandler(
          module.createHandler({ entry: '/client.tsx' }),
          'http://localhost:5173',
        )(req, res);
      } catch (error) {
        next(error);
...
export default defineConfig({
  ...
  plugins: [
    kanso({
      buildId: process.env.KANSO_BUILD_ID ?? 'development',
      routes: {
        '/': '/examples/lab/pages/Home.tsx',
        '/data': '/examples/lab/pages/Data.tsx',
        '/lazy': '/examples/lab/pages/Lazy.tsx',
      },
    }),
    dataServer,
  ],
