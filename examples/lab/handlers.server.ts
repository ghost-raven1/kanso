...
export const handlers: Record<string, RouteHandlers> = {
  'seo-example': { loader: ({ params }) => ({ name: `SEO example: ${params.slug}`, description: 'Metadata resolved from the same loader snapshot as the page.' }) },
  data: {
...
export const sitemapEntries = async () => [{ url: '/seo/example/first' }, { url: '/seo/example/second' }];
