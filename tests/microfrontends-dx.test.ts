...
async function doctorFixture() {
  const root = await temporary();
  const pkg = { name: 'platform', version: '1.0.0', dependencies: { '@kanso/core': '0.8.1', '@kanso/microfrontends': '0.8.1' }, devDependencies: { '@kanso/vite': '0.8.1', vite: '8.3.0' } };
  await writeFile(join(root, 'package.json'), JSON.stringify(pkg));
  await installManifest(root, '@kanso/core', '0.8.1', { 'solid-js': '1.9.15' });
  await installManifest(root, '@kanso/app', '0.8.1', { '@kanso/core': '0.8.1', '@solidjs/router': '0.16.3' });
  await installManifest(root, '@kanso/microfrontends', '0.8.1', { '@kanso/core': '0.8.1', '@kanso/app': '0.8.1' });
  await installManifest(root, '@kanso/vite', '0.8.1');
...
it('scaffolds a standalone remote without copying the shell and targets the current release', async () => {
  ...
  const pkg = JSON.parse(await readFile(join(root, 'package.json'), 'utf8'));
  expect(pkg.dependencies['@kanso/core']).toBe('^0.8.1');
  expect(pkg.dependencies['@kanso/microfrontends']).toBe('^0.8.1');
