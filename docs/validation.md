...
DX-проекты создаются в системном временном каталоге, вне workspace. Установка
Vitest использует закреплённый npm 11.12.1 без изменения глобального installer:
Node 22.23.2/npm 10.9.8 воспроизводимо падает внутри Arborist (`edgesOut`) на
этом peer-графе. Все peer-проверки остаются включёнными; Node 22 с npm 11
проходит ту же чистую установку.
...
`npm run check`: 299/299 Vitest, TypeScript, production-сборки и проверка отсутствия React и лишнего runtime. Новые проверки покрывают layout timing после клиентской навигации и условного монтирования DOM, порядок layout/обычных эффектов, очистку object/callback refs, imperative handles с одинаковыми зависимостями, именованные forwardRef-компоненты, вложенный setup строк, затенённые функции, безопасные config helpers и отказ от записи при source mappings.
