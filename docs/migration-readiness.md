...
| --- | --- | --- |
| P0.2 | Binding внешних stores с selector/equality, server snapshot; фабрика сервисов на приложение/SSR-запрос | Два запроса не разделяют state/promises; начальные данные принимаются без повторного fetch; изменяется только зависимый DOM; disposal снимает подписки |
...
| P1 | Внешняя invalidation HTML-кеша, request services, cancellable subscriptions | Событие backend сбрасывает нужный кеш; поздний результат не восстанавливает закрытую страницу; cookie/session state остаётся в request scope |
