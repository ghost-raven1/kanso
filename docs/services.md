...
Автоматическая замена Zustand React hooks не реализована. Текущий `migrate` / `doctor` консервативно проверяет React peer dependencies на уровне пакета и может блокировать даже vanilla subpath такого пакета. Runtime-поддержка `zustand/vanilla` не отменяет эту диагностику; точный аудит независимых subpath exports остаётся следующей доработкой. Не подключайте React-версию hook и не обходите проверку переименованием импорта.
