...
try {
  ...
  for (const [name, engine] of Object.entries({ chromium, firefox, webkit })) {
    ...
    try {
      ...
      await page.getByRole('link', { name: '06 · Lifecycle & portals' }).click();
      ...
      await page.getByLabel('Portal draft').fill('Keep portal draft');
      await page.getByRole('button', { name: 'Portal count: 0' }).click();
      await page.getByRole('button', { name: 'Portal theme: light' }).click();
      assert.equal(await page.getByRole('complementary', { name: 'Portal example' }).getAttribute('data-theme'), 'dark');
      assert.equal(await page.getByLabel('Portal draft').inputValue(), 'Keep portal draft');
      assert.equal(await page.getByRole('button', { name: 'Portal count: 1' }).count(), 1);
      await page.getByRole('button', { name: 'Close portal' }).click();
      assert.equal(await page.getByRole('button', { name: 'Open portal' }).evaluate(node => node === document.activeElement), true);
      await page.getByRole('button', { name: 'Open portal' }).click();
      assert.equal(await page.getByRole('button', { name: 'Portal count: 0' }).count(), 1);
      await page.getByRole('link', { name: '01 · Reactivity' }).click();
      await page.locator('#increment').waitFor();
      assert.equal(await page.getByRole('complementary', { name: 'Portal example' }).count(), 0);
